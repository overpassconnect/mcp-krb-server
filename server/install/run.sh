...
if [ "$SERVE_CLIENT" = yes ] || [ -n "$CLIENT_EXPORT" ]; then
    ...
    BUNDLE_FILES="setup.sh setup.ps1 setup-macos.sh install-bridge.sh install-anchor.sh JsoncEdit.ps1 uninstall.sh uninstall.ps1 bridge/mcp-krb-bridge.py bridge/mcp-krb-remote-bridge.py bridge/mcp-fetch bridge/krb-git bridge/mcp-krb web/index.html web/app.js web/config.example.js"
