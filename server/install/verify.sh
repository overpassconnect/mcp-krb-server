...
if [ "$SERVE_CLIENT" = no ]; then
    ...
    for _f in setup.sh setup.ps1 install-bridge.sh install-anchor.sh JsoncEdit.ps1 mcp-krb-bridge.py mcp-krb-remote-bridge.py krb-fetch mcp-fetch krb-git mcp-krb index.html app.js config.js; do
